...
/////////////////////////////////////////////////////////////////////////////////////////////////////
                char plain_text_storage[80000]; //storage for encrypted text to be received
...
//Decrypt//
/////////////////////////////////////////////////////////////////////////////////////////////////////
                char dec_text[80000]; //store decrypted text in buffer to send to client
                memset(dec_text, '\0', sizeof(dec_text)); //reset buffer
                ...
                for(int i = 0; i < len_sender; i++){ //iterate through length of text stream
                    char dec;
                    char pt = plain_text_storage[i]; //char in encrypted text
                    ...
                    if(((pt - 64) - (k - 64)) < 0){ //requires mod calc
                        dec = (((pt - 64) - (k - 64)) + 27) + 64;
                    }else{
                        dec = ((pt - 64) - (k - 64)) + 64;
                    }
                    if(dec == 64){ //check if encryption should be space
                        dec = 32;
                    }dec_text[i] = dec; //store new char in buffer
                }
                int len_dec = strlen(dec_text); //length of decrypted stream
/////////////////////////////////////////////////////////////////////////////////////////////////////

//Send New Text to Client//
/////////////////////////////////////////////////////////////////////////////////////////////////////
                if(strcmp(type_storage, "dec") == 0){ //send to client only if connected to correct dec client
                    send(connectionSocket, &len_dec, sizeof(len_dec), 0); //send length of decrypted stream
                    transmit(connectionSocket, dec_text, len_dec, 1); //send decrypted stream to client
                }
